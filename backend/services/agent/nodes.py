...
# 2. Condition: Should the LLM use RAG tool or LLM tool?
def should_use_tool_condition(state):
    """Determine if RAG tool or LLM tool should be used"""
    ...
    1. RAG_REQUIRED - Questions that need specific information from a knowledge base (facts, standards, procedures, specific domain knowledge, documentation lookup)
    2. LLM_TOOL - Questions that need general reasoning, analysis, calculations, creative responses, or can be answered using general knowledge without requiring specific knowledge retrieval
    ...
    - "Explain the concept of machine learning" → LLM_TOOL
    - "What does section 5.2 of the manual say?" → RAG_REQUIRED

    Answer with only one of: RAG_REQUIRED or LLM_TOOL"""  # noqa
...
# 4. LLM tool node - now generates direct response
def call_llm_tool(state):
    """Use LLM as a tool for reasoning, analysis, or creative tasks - returns direct response"""
